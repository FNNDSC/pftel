...
class PFdb():
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        PFdb.dobj_DB.T.touch(
            '/home/dicom/telmetryService/default/info',
            PFdb.d_telemetryCore
        )

    def telemetryService_listObjs(self)-> list:
        return list(PFdb.dobj_DB.T.lstr_lsnode('/DB/telemetryService'))
    ...
    def telemetryService_initObj(self,
        ...
        if str_objName not in self.telemetryService_listObjs():
            PFdb.dobj_DB.T.mkdir('/DB/telemetryService/%s'   % str_objName)
            PFdb.dobj_DB.T.touch(
                    '/DB/telemetryService/%s/json_created'   % str_objName,
        ...
        PFdb.dobj_DB.T.touch('/DB/telemetryService/%s/info' % str_objName, d_data)
        PFdb.dobj_DB.T.touch(
                '/DB/telemetryService/%s/json_modified'      % str_objName,
                json.dumps({'time' : '%s' % datetime.now()})
        )
        return {
            'info'          : PFdb.dobj_DB.T.cat(
                                '/DB/telemetryService/%s/info'           % str_objName
                            ),
